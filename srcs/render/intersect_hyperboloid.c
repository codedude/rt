...
t_float		intersect_hyperboloid(t_ray *ray, t_object *obj, t_float *t)
{
	...
	abc[1] = 2.0 * (ray->origin.x * ray->dir.x + ray->dir.z * ray->origin.z - obj->radius * ray->dir.y * ray->origin.y);
	abc[2] = ray->origin.x * ray->origin.x + ray->origin.z * ray->origin.z - obj->radius * ray->origin.y * ray->origin.y - 1;
