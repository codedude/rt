...
{
	if ((inter.obj_coord.x < 0 && inter.obj_coord.y < 0) || (inter.obj_coord.x > 0 && inter.obj_coord.y > 0))
        {    
            if ((int)fabs(inter.obj_coord.x) % 10 < 5 && ((int)fabs(inter.obj_coord.y) % 10 < 5))
                return (VEC_INIT(0.0, 0.0, 0.0));
            if (!((int)fabs(inter.obj_coord.x) % 10 < 5 || ((int)fabs(inter.obj_coord.y) % 10 < 5)))
                return(VEC_INIT(0.0, 0.0, 0.0));
        }
...
t_vec		marble1(t_float t, t_inter inter)
{
	...
	x = 0.0;
  	...
  	x = sin(inter.obj_coord[0] + cos(inter.obj_coord[1] + 8.0 * t * M_PI) + 6.0 * t * M_PI);
 	x = sqrt(x+1) * 0.7071;
